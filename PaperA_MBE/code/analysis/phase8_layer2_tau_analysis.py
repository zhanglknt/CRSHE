...
for source_name, col in tau_sources.items():
    ...
    # Mann-Whitney U: RD > GD (LOO, non-circular)
    if len(rd_tau_loo) > 0 and len(gd_tau_loo) > 0:
        u_loo, p_loo = stats.mannwhitneyu(rd_tau_loo, gd_tau_loo, alternative="greater")
    ...
    # Circular reference (full v7)
    if len(rd_tau_circ) > 0 and len(gd_tau_circ) > 0:
        u_circ, p_circ = stats.mannwhitneyu(rd_tau_circ, gd_tau_circ, alternative="greater")
...
for tissue_col in gtex_tissue_cols:
    ...
    # Mann-Whitney: RD > GD for tissue specificity
    if len(tissue_specs_rd) > 5 and len(tissue_specs_gd) > 5:
        u, p = stats.mannwhitneyu(tissue_specs_rd, tissue_specs_gd, alternative="greater")
